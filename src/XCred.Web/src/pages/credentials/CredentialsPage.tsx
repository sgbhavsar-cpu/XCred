...
export default function CredentialsPage() {
  ...
  const fetchFolders = async () => {
    try {
      ...
      setFolders(flattenFolders(res.data.data));
    } catch { /* the bulk-edit folder picker just stays empty; not worth a toast */ }
  };

  useEffect(() => { fetchGroups(); fetchFolders(); }, []);
  ...
  const { sensors, active, handleDragStart, handleDragEnd } = useCredentialDnd(async (kind, credId, groupId) => {
    ...
    try {
      await bulkAssign([credId], { updateCredentialGroup: true, credentialGroupId: groupId });
      toast.success(groupId ? 'Moved to credential group.' : 'Removed from credential group.');
  ...
  return (
    <div className="p-6">
        ...
        <DndContext sensors={sensors} collisionDetection={pointerWithin} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100 overflow-hidden">
            {visibleGroups.map(group => {
              ...
              return (
                <div key={group.id}>
                  <DroppableRow id={group.id} onClick={() => toggleExpand(group.id)}
      ...
      {showBulkEdit && (
        <BulkEditModal count={selectedIds.size} folders={folders} groups={groups}
